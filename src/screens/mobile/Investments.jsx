...
import { PQCard, PQAmount, PQPill, PQEyebrow, PQBar } from '../../components/ui';
...
export default function Investments() {
  ...
  return (
    <MobileScreen active="inv" bg={theme.bgOff}>
      <div style={{ height: '100%', overflowY: 'auto' }}>
        <div style={{ padding: '14px 22px 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8, alignItems: 'center' }}>
                <PQPill theme={theme} bg={theme.bg} color={theme.positive}>
                  ↑ R$ 312,40 (1,11%) · mês
                </PQPill>
            ...
            <div style={{ display: 'flex', gap: 18, alignItems: 'center' }}>
              <div style={{ width: 110, height: 110, borderRadius: 55, flexShrink: 0,
                ...
                <div style={{ position: 'absolute', inset: 16, borderRadius: 50,
                  ...
                  <span style={{ fontFamily: PQ_T.display, fontSize: 22, color: theme.positive }}>+9,4%</span>
                </div>
              </div>
              <div style={{ flex: 1 }}>
                {allocs.map((a, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0' }}>
                    ...
                    <div style={{ fontFamily: PQ_T.mono, fontSize: 11, color: theme.mute }}>{a.pct}%</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <PQCard theme={theme} inset={false} style={{ marginTop: 14, padding: 14,
            background: theme.ink, color: theme.bg, border: 'none', overflow: 'hidden', position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: 10.5, letterSpacing: '0.18em', textTransform: 'uppercase',
                  opacity: 0.6, fontWeight: 600 }}>Reserva de emergência</div>
          ...
          <div style={{ marginTop: 20, paddingBottom: 8 }}>
            <PQEyebrow theme={theme} right={
              <span style={{ color: theme.accent, cursor: 'pointer' }}>+ aplicar</span>
            }>Aplicações</PQEyebrow>
            <PQCard theme={theme} inset={false}>
              {holdings.map((h, i, a) => (
                <div key={i} onClick={() => navigate('/investimentos/detalhe')}
                  style={{ display: 'flex', alignItems: 'center', gap: 12,
                  padding: '12px 14px',
                  borderBottom: i < a.length-1 ? `0.5px solid ${theme.hairSoft}` : 'none',
                  cursor: 'pointer' }}>
                  <div style={{ width: 6, height: 36, borderRadius: 3, background: h.color, flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10.5, color: theme.mute, marginTop: 2, letterSpacing: '0.04em' }}>{h.kind}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontFamily: PQ_T.mono, fontSize: 13, color: theme.ink, fontWeight: 500 }}>
                      R$ {h.v.toLocaleString('pt-BR')}
                    </div>
