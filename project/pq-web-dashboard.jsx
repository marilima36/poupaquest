// Web · Dashboard — 1280×800 layout
// Sidebar à esquerda, conteúdo principal denso, rail direito com Poupi/jornada

function WebSidebar({ theme }) {
  const nav = [
    { l: 'Início',         I: IWallet, active: true },
    { l: 'Lançamentos',    I: IBolt },
    { l: 'Gastos',         I: IPie },
    { l: 'Investimentos',  I: IInvest },
    { l: 'Jornada',        I: ITarget },
    { l: 'Recorrências',   I: IRepeat },
  ];
  return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {nav.map((n, i) => (
          <div key={i} style={{
            ...
            background: n.active ? theme.bgOff : 'transparent',
            color: n.active ? theme.ink : theme.mute,
            fontSize: 13, fontWeight: n.active ? 600 : 500,
            cursor: 'pointer',
          }}>
      ...
      <div style={{ padding: 14, borderRadius: 14,
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between',
            fontSize: 10, color: theme.mute, marginBottom: 4,
...
// Sparkline (gastos diários no mês) — barras pequenas
function WebDailySpend({ theme }) {
  const data = [120, 60, 0, 180, 40, 220, 90, 150, 70, 30, 0, 240, 120, 60, 80,
    140, 90, 110, 0, 220, 80, 60, 130, 0, 90, 180, 140, 200, 60, 80];
    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 100,
      ...
      {data.map((v, i) => (
        <div key={i} style={{
          ...
          minHeight: 2, borderRadius: 1.5,
          background: i === 14 ? theme.accent : theme.ink,
          opacity: i > 14 ? 0.15 : 1,
        }} />
      ))}
    </div>
...
function WebDashboard({ theme }) {
  ...
  return (
    <div style={{ height: '100%', background: theme.bgOff,
      display: 'flex', overflow: 'hidden', color: theme.ink,
      fontFamily: PQ_T.sans }}>

      <WebSidebar theme={theme} />

      {/* Main */}
      <div style={{ flex: 1, padding: '20px 28px', overflow: 'auto', display: 'flex',
        flexDirection: 'column', gap: 20 }}>

        {/* Top bar */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontFamily: PQ_T.display, fontSize: 26, lineHeight: 1.05,
              letterSpacing: '-0.02em' }}>Bom dia, Lívia.</div>
            <div style={{ fontSize: 12.5, color: theme.mute, marginTop: 4 }}>
              15 de maio · quinta-feira · você está na metade do mês.
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8,
              padding: '8px 14px', background: theme.bg, borderRadius: 10,
              border: `0.5px solid ${theme.hair}`, fontSize: 13, color: theme.mute, width: 240 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: 14 }}>
          <PQCard theme={theme} style={{ padding: 18, background: theme.ink,
            color: theme.bg, border: 'none', position: 'relative', overflow: 'hidden' }}>
            <div style={{ fontSize: 10.5, letterSpacing: '0.16em',
              textTransform: 'uppercase', opacity: 0.6, fontWeight: 600 }}>Saldo do mês</div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 14, fontSize: 11,
              opacity: 0.85 }}>
              <span>↑ 12% vs abril</span>
              <span>·</span>
              <span>61% da meta de R$ 7.000</span>
            </div>
          ...
          <PQCard theme={theme} style={{ padding: 16 }}>
            ...
            <div style={{ marginTop: 12, fontSize: 11, color: theme.mute }}>
              4 lançamentos · salário + freelas
            </div>
          </PQCard>
          <PQCard theme={theme} style={{ padding: 16 }}>
          ...
          <PQCard theme={theme} style={{ padding: 16 }}>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div style={{ fontSize: 10.5, color: theme.mute, marginTop: 5,
                fontFamily: PQ_T.mono }}>16% da meta · 6 meses</div>
            </div>
          </PQCard>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14 }}>
          <PQCard theme={theme} style={{ padding: 20 }}>
            ...
            <div style={{ marginTop: 18 }}>
              <WebDailySpend theme={theme} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between',
              fontSize: 10.5, color: theme.mute, marginTop: 6, fontFamily: PQ_T.mono }}>
              <span>1 mai</span>
              <span>10</span>
              <span style={{ color: theme.accent, fontWeight: 600 }}>hoje · 15</span>
              <span>20</span>
              <span>31 mai</span>
            </div>
          </PQCard>
          ...
          <PQCard theme={theme} style={{ padding: 18 }}>
            <PQEyebrow theme={theme} right={<span style={{ color: theme.accent }}>todas</span>}>Categorias</PQEyebrow>
              ...
              {cats.map((c, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 11.5, color: theme.ink, fontWeight: 500 }}>
                          {meta.short}
                        </span>
                        <span style={{ fontFamily: PQ_T.mono, fontSize: 11, color: theme.mute }}>
                          R$ {c.used}
                        </span>
                      </div>
        ...
        {/* Bottom row: transactions table + journey */}
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14 }}>
          <PQCard theme={theme} style={{ padding: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between',
              padding: '16px 20px', borderBottom: `0.5px solid ${theme.hairSoft}` }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: theme.ink }}>Últimos lançamentos</div>
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} style={{
                    ...
                    <td style={{ padding: '11px 20px', display: 'flex', alignItems: 'center', gap: 8 }}>
                      {row.c
                        ? <PQCatChip catKey={row.c} theme={theme} size={24} />
                        : <div style={{ width: 24, height: 24, borderRadius: 10,
                      ...
                      {row.r && (
                        <span style={{ background: theme.bgOff, padding: '1px 6px',
                          borderRadius: 5, fontSize: 9, color: theme.mute,
                          fontWeight: 600, letterSpacing: '0.06em' }}>RECUR</span>
                      )}
                    </td>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <PQCard theme={theme} style={{ padding: 18, background: theme.ink, color: theme.bg,
              ...
              <div style={{ fontFamily: PQ_T.display, fontSize: 22, lineHeight: 1.1,
                letterSpacing: '-0.01em' }}>Semana sem delivery</div>
              <div style={{ fontSize: 11.5, opacity: 0.7, marginTop: 4 }}>
                5 de 7 dias · faltam 2
              </div>
              <div style={{ marginTop: 14 }}>
                <PQBar pct={71} theme={theme} height={4}
                  color={theme.accent} bg={'rgba(255,255,255,0.15)'} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between',
                fontSize: 11, marginTop: 8, opacity: 0.8 }}>
                <span>71%</span>
                <span style={{ fontFamily: PQ_T.mono }}>+ 120 XP</span>
              </div>
            </PQCard>
            ...
            <PQCard theme={theme} style={{ padding: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {[
                  ...
                  <div key={i} style={{
                    aspectRatio: '1', borderRadius: 12, padding: 10,
