// Mobile · Recorrências — assinaturas, parcelas, contas mensais

function Recurrings({ theme }) {
  const subs = [
  ...
  return (
    <div style={{ height: '100%', background: theme.bg,
      ...
      <div style={{ padding: '14px 22px 0' }}>
        ...
        {/* Calendar strip */}
        <div style={{ marginTop: 18, padding: 12, background: theme.bgOff,
          borderRadius: 14 }}>
            ...
            {Array.from({ length: 30 }).map((_, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, height: `${Math.max(h, 4)}%`,
                  background: accent ? theme.ink : theme.hair, borderRadius: 1.5,
                  minHeight: 4 }} />
              );
            })}
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between',
            fontSize: 9.5, color: theme.mute, marginTop: 6,
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12,
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontFamily: PQ_T.mono, fontSize: 13, color: theme.ink,
                    fontWeight: 500 }}>R$ {s.v.toFixed(2)}</div>
          ...
          <PQCard theme={theme} inset={false}>
            {fixed.map((f, i, a) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12,
                ...
                <div style={{ fontFamily: PQ_T.mono, fontSize: 13, color: theme.ink,
                  fontWeight: 500 }}>R$ {f.v.toLocaleString('pt-BR', {minimumFractionDigits:2})}</div>
              </div>
            ))}
          </PQCard>
        </div>
        ...
        <div style={{ marginTop: 18, padding: 14, borderRadius: 14,
          ...
          <div style={{ fontSize: 12, color: theme.ink, flex: 1, lineHeight: 1.4 }}>
            ...
            <span style={{ color: theme.accent, fontWeight: 600, marginLeft: 4 }}>Revisar →</span>
          </div>
        </div>
      </div>
...
Object.assign(window, { Recurrings });
