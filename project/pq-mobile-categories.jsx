// Mobile · Categorias / Análise

function CategoriesView({ theme }) {
  const cats = [
  ...
  const segments = cats.map((c) => ({
    color: theme[PQ_CATS[c.k].tone], value: c.used,
  }));
  ...
  return (
    <div style={{ height: '100%', background: theme.bg,
      ...
      <div style={{ padding: '14px 22px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontFamily: PQ_T.mono, fontSize: 14,
              color: theme.positive, fontWeight: 600, marginTop: 4 }}>
              ↓ 6,8%
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 16, display: 'flex', gap: 8,
          ...
          <button style={{ background: 'none', border: 'none', color: theme.ink,
            fontSize: 18, opacity: 0.6, transform: 'rotate(180deg)' }}><IChevR size={18} /></button>
          ...
          <button style={{ background: 'none', border: 'none', color: theme.ink,
            fontSize: 18, opacity: 0.6 }}><IChevR size={18} /></button>
        </div>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between',
            fontSize: 11, color: theme.mute, marginTop: 6,
            fontFamily: PQ_T.mono }}>
            <span>R$ 0</span>
            <span>R$ 5.000 · meta</span>
          </div>
        </div>
            ...
            return (
              <div key={i} style={{ padding: '12px 0',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between',
                    fontSize: 10.5, color: theme.mute, marginTop: 2,
                    fontFamily: PQ_T.mono }}>
                    <span>limite R$ {c.lim.toLocaleString('pt-BR')}</span>
                    <span style={{ color: c.change > 0 ? theme.negative
                      : c.change < 0 ? theme.positive : theme.mute }}>
      ...
      <HomeTabBar theme={theme} active="cat" />
    </div>
  );
}
